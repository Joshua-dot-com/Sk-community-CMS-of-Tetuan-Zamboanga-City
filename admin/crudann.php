...
<div class="row">
<div class="col-md-6">
<h3 class="page-title mb-0">announcement</h3>
</div>
...
if (mysqli_num_rows($run_query) > 0) {
    while ($row = mysqli_fetch_array($run_query)) {
        ...
        // Check if the image path already contains the directory path
        if (strpos($post_image, 'upload/') === 0) {
            // Image path already contains the directory path, use it as is
            $image_path = $post_image;
        } else {
            // Image path doesn't contain the directory path, construct the full path
            $image_path = "upload/" . $post_image;    
        }
        ...
        echo "<td><img width='100' src='$image_path' alt='Post Image'></td>"; // Use updated image path here
        echo "<td>$post_date</td>";
        echo "<td>";
        echo "<a href='edit_post.php' class='btn btn-primary btn-sm editBtn' data-post-id='$post_id'>Edit</a>";
        echo "<a href='#' class='btn btn-danger btn-sm deleteBtn' data-post-id='$post_id'>Delete</a>";
        echo "</td>";
        echo "</tr>";
    }
} else {
...
$(document).ready(function () {
    ...
    $('.editBtn').click(function (event) {
        ...
        var postId = $(this).data('post-id'); // Corrected from 'data-id' to 'data-post-id'

        // Navigate to the edit_post.php page with post ID parameter
        window.location.href = 'edit_post.php?id=' + postId; // Pass postId as a parameter
    });
    ...
    $('.deleteBtn').click(function (event) {
        ...
        if (confirm("Are you sure you want to delete this announcement?")) {
            ...
            $.ajax({
                url: 'delete_post.php', // Replace 'delete_post.php' with the actual file to handle deleting post
                type: 'POST',
                data: { id: postId },
                success: function (response) {
                    // Update UI or redirect to a success page
