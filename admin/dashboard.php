  ...
  <span class="float-left"><img src="assets/img/dash/d1.png" alt="" width="80"></span>
  ...
  echo "<span>TOTAL YOUTH:</span> <h3>$num_rows</h3>";
  mysqli_close($conn);
  ?>
  </div>
  </div>
  ...
  <?php include '../db_conn.php'; 
  $q = mysqli_query($conn, "SELECT * FROM announcement");
  if (!$q) {
      echo "Error: " . mysqli_error($conn);
      exit();
  }
  $num_rows = mysqli_num_rows($q);
  echo "<span>ANNOUNCEMENTS:</span> <h3>$num_rows</h3>";
  mysqli_close($conn);
  ?>
  </div>
  <span class="float-right"><img src="assets/img/dash/d2.png" width="80" alt=""></span>
  </div>
  </div>
  <div class="col-md-6 col-sm-6 col-lg-6 col-xl-3">
  <div class="dash-widget dash-widget5">
  <span class="float-left"><img src="assets/img/dash/d3.png" alt="" width="80"></span>
  <div class="dash-widget-info text-right">
  <?php include '../db_conn.php'; 
  $q = mysqli_query($conn, "SELECT * FROM appointments where status ='approved' ");
  if (!$q) {
      echo "Error: " . mysqli_error($conn);
      exit();
  }
  $num_rows = mysqli_num_rows($q);
  echo "<span>APPOINTMENTS:</span> <h3>$num_rows</h3>";
  mysqli_close($conn);
  ?>
...
// SQL query to calculate age ranges dynamically
$sql = "SELECT 
            CASE
                WHEN TIMESTAMPDIFF(YEAR, birthday, CURDATE()) BETWEEN 13 AND 18 THEN '13-18'
                WHEN TIMESTAMPDIFF(YEAR, birthday, CURDATE()) BETWEEN 19 AND 25 THEN '19-25'
                WHEN TIMESTAMPDIFF(YEAR, birthday, CURDATE()) BETWEEN 26 AND 30 THEN '26-30'
                WHEN TIMESTAMPDIFF(YEAR, birthday, CURDATE()) BETWEEN 31 AND 35 THEN '31-35'
                ELSE 'Other'
            END AS age_range,
            COUNT(*) AS count
        FROM 
