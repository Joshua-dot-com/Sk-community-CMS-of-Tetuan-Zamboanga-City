<?php
  ...
  <div class="row">
  <div class="col-md-6">
  <h3 class="page-title mb-0">Borrowed Items</h3>
  </div>


  <div class="container mt-5">
                <table class="table" id="borrowedItemsTable">
                    <thead>
                    <tr>
                        <th>#</th>
                        <th>Name</th>
                    ...
                    // Query to fetch borrowed items with user details
                    $sql = "SELECT borrow.borrow_id, users.first_name, users.last_name, items.name AS item_name, borrow.quantity, borrow.borrow_date, borrow.return_date FROM borrow
                            INNER JOIN users ON borrow.user_id = users.id
                            INNER JOIN items ON borrow.item_id = items.id";
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo "<td><button class='btn btn-primary' onclick='returnItem(".$row["borrow_id"].", this.parentNode.parentNode.rowIndex)'>Returned</button></td>";
                            echo "</tr>";
                        }
                    } else {
...
<script>
    $(document).ready(function() {
        // Check return date for each row
        $('#borrowedItemsTable tbody tr').each(function() {
            // Get the return date cell content
            var returnDate = $(this).find('td:eq(6)').text();
            // Convert return date to a JavaScript Date object
            var returnDateObj = new Date(returnDate);
            // Get today's date
            var today = new Date();
            // Compare return date with today's date
            if (returnDateObj.toDateString() === today.toDateString()) {
                // Add CSS class to mark the row as overdue
...
$(document).ready(function() {
    ...
    // Filter rows with today's return date
    var todayRows = $('#borrowedItemsTable tbody tr').filter(function() {
        // Get the return date cell content
        var returnDate = $(this).find('td:eq(6)').text();
        // Return true if the return date is today, false otherwise
        return returnDate === todayDateString;
    });

    // Move today's rows to the top of the table
    todayRows.prependTo('#borrowedItemsTable tbody');
});
</script>


<script>
function returnItem(borrowId, rowIndex) {
    var confirmation = confirm("Are you sure that this item is bieng returned?");
    if (confirmation) {
        // Send an AJAX request to return the item
        $.ajax({
            url: 'return.php',
            type: 'POST',
            data: { borrow_id: borrowId },
            dataType: 'json', // Specify JSON data type for proper response handling
            success: function(response) {
                if (response.success) {
                    // Remove the corresponding row from the table
                    $('#borrowedItemsTable').find('tr:eq(' + rowIndex + ')').remove();
                    // Display success message
                    alert("Item returned successfully!");
                } else {
                    // Display error message
                    alert("Failed to return item. " + response.message);
                }
            },
...
function updateItemQuantity(itemId, quantity) {
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                // Update quantity in the item table was successful
                console.log("Quantity updated successfully for item ID: " + itemId);
            } else {
                // Display error message
                alert("Failed to update item quantity. " + response.message);
            }
        },
        error: function(xhr, status, error) {
            // Display error message
